...
def run_script():
    ...
    if not image or not region:
        messagebox.showerror("Input Error", "Please provide the required inputs: Image and Region.")
        return
...
tk.Button(root, text="Run", command=run_script).grid(row=5, column=1, pady=20)
