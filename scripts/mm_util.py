...
def estimate_chunk_size(
    ...
    if device.type in ['cpu', 'mps']:
        # For CPU/MPS, use conservative default chunk size
        chunk_size = 50
        device_name = "MPS (Apple Silicon)" if device.type == 'mps' else "CPU"
        logging.info(f"{device_name} detected, using default chunk size: {chunk_size} slices")
        return chunk_size
...
def _init_wandb(image_path, device, chunk_size, overlap, precision, region):
    ...
    try:
        ...
        available_ram = 0  # Removed psutil dependency
